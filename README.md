...
| GF_FEATURE_TOGGLES_ENABLE        | Включает функцию публичных дашбордов в Grafana. Это позволяет делиться дашбордами с другими пользователями, не требуя от них авторизации, значение равно publicDashboards                     |
| `GF_PLUGIN_GRAFANA_IMAGE_RENDERER_RENDERING_IGNORE_HTTPS_ERRORS         | Эта переменная позволяет Grafana игнорировать ошибки HTTPS при рендеринге изображений. Это может быть полезно, если вы используете не-HTTPS-сервер для хранения изображений, значение равно true                    |
| GF_DATABASE_SSL_MODE        | Отключает использование SSL для подключения к базе данных PostgreSQL. Значение равно "disable"         |   
| GF_DATABASE_TYPE         | Значение "postgres" указывает, что Grafana будет использовать PostgreSQL в качестве базы данных           |
| GF_DATABASE_HOST         | Адрес хоста базы данных. Например "postgres" значение равно "grafanadb:5432"           |
