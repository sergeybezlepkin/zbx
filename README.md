...
| POSTGRES_USER, POSTGRES_USER_FILE, POSTGRES_PASSWORD, POSTGRES_PASSWORD_FILE   | ЭТИ ПЕРЕМЕННЫЕ ПРЕДСТАВЛЯЮТ СОБОЙ IP- ИЛИ DNS-ИМЯ СЕРВЕРА POSTGRESQL. ПО УМОЛЧАНИЮ ЗНАЧЕНИЕ — «POSTGRES-SERVER».  |
| POSTGRES_DB   | ПЕРЕМЕННАЯ — ЭТО ИМЯ БАЗЫ ДАННЫХ ZABBIX. ПО УМОЛЧАНИЮ ЗНАЧЕНИЕ — ZABBIX.  |
| DB_SERVER_SCHEMA   | ПЕРЕМЕННАЯ, КОТОРАЯ ОПРЕДЕЛЯЕТ НАЗВАНИЕ СХЕМЫ БАЗЫ ДАННЫХ НА СЕРВЕРЕ.  |
| ENABLE_TIMESCALEDB   | ПЕРЕМЕННАЯ, КОТОРАЯ УКАЗЫВАЕТ НА НЕОБХОДИМОСТЬ ВКЛЮЧЕНИЯ РАСШИРЕНИЯ TIMESCALEDB ДЛЯ БАЗЫ ДАННЫХ.  |
| POSTGRES_USE_IMPLICIT_SEARCH_PATH   | ПЕРЕМЕННАЯ, КОТОРАЯ УКАЗЫВАЕТ, СЛЕДУЕТ ЛИ ИСПОЛЬЗОВАТЬ НЕЯВНЫЙ ПОИСК ПУТИ В POSTGRESQL. КОГДА ЗНАЧЕНИЕ ЭТОЙ ПЕРЕМЕННОЙ УСТАНОВЛЕНО НА "FALSE", БАЗА ДАННЫХ POSTGRESQL НЕ БУДЕТ ИСПОЛЬЗОВАТЬ НЕЯВНЫЙ ПОИСК ПУТИ ДЛЯ ПОИСКА ОБЪЕКТОВ БАЗЫ ДАННЫХ. ЭТО МОЖЕТ БЫТЬ ПОЛЕЗНО ДЛЯ ТОГО, ЧТОБЫ ИЗБЕЖАТЬ ПУТАНИЦЫ И КОНФЛИКТОВ ПРИ РАБОТЕ С ОБЪЕКТАМИ В РАЗНЫХ СХЕМАХ.  |
