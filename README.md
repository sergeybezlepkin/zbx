# Запускаем Zabbix. Переходим на контейнеры: быстрый запуск, экономия ресурсов и крепкий сон

![logo](images/cover_last.png)
...
[zbx/env_vars/env_db](env_vars/env_db)
...
[zbx/els/config/env_zbxelastic](els/config/env_zbxelastic)

| Переменная | Описание |
|-----------------------|-------------------------------------------------------------------------------------------|
| cluster.name| Имя кластера Elasticsearch. Устанавливает уникальное имя для кластера, чтобы он мог быть идентифицирован в сети. |
| node.name | Имя узла Elasticsearch. Устанавливает уникальное имя для данного узла в кластере, а также указывает порт, на котором узел будет прослушивать соединения. |
| cluster.initial_master_nodes | Указывает узлы, которые будут изначально избраны в качестве мастер-узлов кластера. В данном случае, только один узел `zbxelastic:9200` будет изначальным мастером. |
| ingest.geoip.downloader.enabled | Отключает загрузку базы данных GeoIP для обработки геолокационных данных в ингест-пайплайне. Это может быть полезно для экономии ресурсов, если вы не используете геолокационные данные. |
| xpack.security.enabled | Отключает X-Pack Security, чтобы Elasticsearch работал в открытом режиме без авторизации. Это позволяет доступ к Elasticsearch без паролей или других механизмов безопасности. |
| cluster.routing.allocation.disk.watermark.enable_for_single_data_node | Включает использование дисковых водяных знаков для управления использованием дискового пространства в кластере. Это позволяет автоматически перемещать индексы между узлами, чтобы избежать переполнения диска. |
| transport.host | Устанавливает IP-адрес, на котором Elasticsearch будет прослушивать соединения. В данном случае, установлено значение `127.0.0.1`, что означает, что Elasticsearch будет доступен только на локальной машине. |
| xpack.monitoring.collection.enabled | Включает сбор метрик и статистики для мониторинга производительности и состояния кластера Elasticsearch. Это позволяет использовать Kibana для визуализации и анализа метрик. |

#### Сервер Zabbix

[zbx/env_vars/env_zbxsrv](env_vars/env_zbxsrv)
...
[zbxproxy/env_vars/env_zbxproxy](zbxproxy/env_vars/env_zbxproxy)
...
[zbx/env_vars/env_web](env_vars/env_web)
...
[zbx/env_vars/env_agent2](env_vars/env_agent2)
...
[zbx/env_vars/env_java](env_vars/env_java)
...
[zbx/env_vars/env_snmp](env_vars/env_snmp)
...
[zbx/env_vars/env_webservice](env_vars/env_webservice)
...
[zbx/nginx/config/env_nginx](nginx/config/env_nginx)
...
[zbx/config/env_grafana](grafana/config/env_grafana)

| Переменная | Описание |
|-----------------------|-------------------------------------------------------------------------------------------|
| GF_SERVER_ROOT_URL         | Эта переменная определяет корневой URL для вашего экземпляра Grafana. Например http://localhost:3000            |
| GF_LOG_LEVEL         | Устанавливает уровень логирования для Grafana. Варианты: "trace", "debug", "info", "warn" и другие                     |
| GF_FEATURE_TOGGLES_ENABLE   | Включает функцию публичных дашбордов в Grafana. Это позволяет делиться дашбордами с другими пользователями, не требуя от них авторизации  |
| `GF_PLUGIN_GRAFANA_IMAGE_RENDERER_RENDERING_IGNORE_HTTPS_ERRORS         | Эта переменная позволяет Grafana игнорировать ошибки HTTPS при рендеринге изображений. Это может быть полезно, если вы используете не-HTTPS-сервер для хранения изображений                  |
| GF_DATABASE_SSL_MODE        | Отключает использование SSL для подключения к базе данных PostgreSQL.       |   
| GF_DATABASE_TYPE         | Значение "postgres" указывает, что Grafana будет использовать PostgreSQL в качестве базы данных           |
| GF_DATABASE_HOST         | Адрес хоста базы данных.        |
| GF_DATABASE_USER         | Имя пользователя, используемого для подключения к базе данных            |
| GF_DATABASE_NAME         | Имя базы данных           |

#### Kibana

[zbx/grafana/config/env_grafana](kibana/config/env_kibana)

 | Переменная                | Описание                                                                                   |
|---------------------------|---------------------------------------------------------------------------------------------|
| ELASTICSEARCH_HOSTS   | URL хоста Elasticsearch. В данном случае указан адрес `http://zbxelastic:9200` для подключения. |
| XPACK_MONITORING_ENABLED| Флаг, включающий мониторинг с помощью X-Pack. Значение `true` активирует мониторинг.          |
| XPACK_REPORTING_ROLES_ENABLED | Флаг, включающий роли для отчетов в X-Pack. Значение `false` отключает эту функциональность.   |
| XPACK_SECURITY_SESSION_IDLETIMEOUT | Таймаут неактивной сессии в безопасности X-Pack. Установлено значение `20m` (20 минут).       |
| XPACK_SECURITY_SESSION_LIFESPAN | Продолжительность жизни сессии в безопасности X-Pack. Установлено значение `30d` (30 дней).     |
...
docker compose up -d
```
Ждем пару минут, мониторинг готов. Переходи по своему IP-адресу на порт 80. Запущен сервер Zabbix, база данных Zabbix, веб-сервер Zabbix, ловушка Zabbix, шлюз Zabbix, отправка отчетов Zabbix. Если нужен агент-контейнер убираем # в файле, и запускаем повторно. Агент-контейнер запускается без внешнего порта, почему? База данных Zabbix и система поиска Elasticsearch тоже не имеют внешнего порта на подключение и скрыты по контуру сети, конечно в целях безопасности. Если нужен агент с внешним подключением всегда можно установить как службу на узле, и осуществлять мониторинг узла. Да у меня 2 агента на узле, но я получаю разграничение по мониторингу узла и внутренних компонентов, все это в целях безопасности, но может это через чур? Вот еще дополнительная настройка функциональности агента-контейнера, например, нужен мониторинг платформы Docker, но при добавлении шаблона, получаю ошибку в недостаточности прав доступа к сокету Docker, ниже сценарий исправит эту проблему. 

```sh
#!/bin/bash
grep docker /etc/group | sed "s/\($USER\)/zabbix/" > docker_group.txt
echo $(cat docker_group.txt) | docker exec -i -u root zbxagent2 /bin/sh -c 'cat >> /etc/group' > /dev/null || { echo "Failed to modify etc/group"; exit 1; }
if [ $? -eq 0 ] ; then
  echo "Commands executed successfully inside Docker container"
else
  echo "Commands failed inside Docker container"
fi
docker restart zbxagent2
docker ps -a | grep zbxagent2
exit
```

Остальные компоненты: система поиска Elasticsearch, прокси-сервер Nginx, панель визуализации Kibana, панель визуализации Grafana по умолчанию запущены не будут, их запуск ищи через содержание. Прокси-сервер я о нем не забыл, идет далее. Доступ по умолчанию `Admin` с паролем `zabbix`.

# Журналирование

![doozle](images/doozle.png)

В каталоге проекта есть каталог logs/. Там лежит файл [compose.yml](logs/compose.yml) для запуска отличного открытого ПО по сбору журналов от контейнеров. Сбор идет в режиме реального времени, а наблюдать, анализировать данные можно из браузера. Проект открытый с [ним можно ознакомится тут.](https://dozzle.dev/). Внутри кода есть перменная `DOZZLE_HOSTNAME: Your_IP` - измени на свой IP-адрес узла.

Переходи в каталог и запускай: 

```sh
docker compose up -d 
```
После запуска, переходи по своему IP-адресу на порт 3080. 
...
Книг по изучению данной системы на русском языке почти и нет, у меня есть вот эти 2, например: [”Elasticsearch. Kibana. Logstash и поисковые системы нового поколения”](https://goo.su/mHjSE), или [“Активное выявление угроз с Elastic Stack”](https://goo.su/ZEDDMw). Рекомендую ролики [с официального Youtube канала](https://goo.su/bCuMUJ) разработчика Elastic. Прежде чем запустить проект делаю настройки конфигурации как написано [в документации Elasticsearch](https://www.elastic.co/guide/en/elasticsearch/reference/7.17/system-config.html), нужные настройки ниже, т.к. будет запущен контейнер.
...
# Запускаем кластер Zabbix

![zabbix_ha](images/zabbix_ha1_new.png)

У ПО Zabbix есть режим кластера “высокой доступности”. Что это такое? Это запуск двух и более серверов, к одной базе данных, и в случае сбоя первого, в работу включается второй север. Все это сведет простой работы системы мониторинга к нулю. 

Вот информация из [документации:](https://www.zabbix.com/documentation/6.0/ru/manual/concepts/server/ha) 
...
Для запуска буду использовать еще один инструмент, это - [Docker Swarm](https://docs.docker.com/engine/swarm/). Swarm представляет из себя инструмент для объединения разных платформ (демонов) Docker в кластер, называемый [роем.](https://docs.docker.com/engine/swarm/key-concepts/) Он так же работает по принципу “клиент-сервер” где выделяет машину(ы) - мастер-узел, и машины - рабочие узлы, или являясь тем и другим, а взаимодействие идет по протоколу HTTP. Мастер-узел управляет рабочими узлами и распределяет запуск контейнеров по доступным узлам (ресурсам), отслеживает их состояние, а в случаи сбоя перезапускает службу, обновляет конфигурацию, поддерживая заданное состояние. Кластер открывает возможность запускать копии (реплики) сервисов на разных узлах в рое, тем самым балансируя нагрузку между запущенными контейнерами, и дает возможность масштабировать сервис. До этой строки, я не знакомил тебя с сетевой организацией, время пришло, ты можешь почитать [здесь](https://docs.docker.com/network/), в сети много разных статей, я кратко еще раз опишу. На платформе Docker сетевое взаимодействие представляет собой возможность контейнеров подключаться по сети друг с другом. 

Эту функциональность обеспечивает сетевой драйвер, и он тут не один:

- **Bridge** (по умолчанию): обеспечивает изоляцию контейнеров внутри хоста и позволяет им общаться между собой;
- **Host**: контейнер использует сетевые настройки машины,, что делает его доступным для других устройств в сети;
- **Overlay**: позволяет создавать сети, которые распространяются на несколько узлов в кластере Docker;
- **Macvlan**: позволяет привязывать контейнеры к внешнему сетевому интерфейсу и назначать им специфический MAC-адрес;
- **None**: контейнер не имеет сетевого доступа;

Есть еще так называемые сетевые плагины, [о них можно здесь](https://docs.docker.com/engine/extend/plugins_network/).

Запуск системы мониторинга до кластера был через `сеть Bridge (мост)`, запуская кластер высокой доступности Zabbix буду использовать `сеть Overlay`. Когда я активирую режим Swarm, [данный режим включит](https://docs.docker.com/engine/swarm/swarm-tutorial/#the-ip-address-of-the-manager-machine) `сеть Overlay` и контейнеры будут запущены по машинам в кластере, и смогут общаться между друг другом по сети. Для запуска кластера нужно [3 машины с платформой Docker](https://docs.docker.com/engine/swarm/swarm-tutorial/). Буду называть машина 1, машина 2, машина 3. Мое окружение: Машина 1 (Oracle Linux), машина 2 (Ubuntu 22.04), машина 3 (Ubuntu 22.04). 
Перед запуском Swarm-режима, версия платформы Docker на машинах должна быть одинаковой, например версия 26.X.X. 
Машины должны видеть друг друга по сети. Для коммуникации платформа Docker использует:

- **Порт 2377/tcp** - панель управления Swarm-режима;
- **Порт 7946/tcp** -  для связи узлов в сети;
- **Порт 4789/udp** - для трафика Overlay-сети. 

На машине 1 - будет мастер-узлом, тут `Oracle Linux 9` добавлю правило в брандмауэр, открываю порты: `2377/tcp`, `7946/tcp`, `4789/udp`
...
Машина 2 и 3 будут рабочими-узлами, помню что тут `Ubuntu 22.04` открываю порты на машинах:
...
Проверяем кластер и узлы, запускаем команду на мастер-узле: 

```sh
docker node ls
```
И видим список из машин. Кто из них менеджер, а кто рабочий, версию ядра Docker, и состояние. Платформы Docker обьеденены в кластер, теперь запускаем кластер Zabbix. Для удобства отслеживания состояния контейнеров запущу [сервис журналирования](#Журналирование) на каждой машине.
