...
docker compose up -d
```
Для веб-интерфейса Zabbix не открыт внешний порт, сделаем перенаправление и получим доступ через порт 80. Переходим на свой адрес, порт 3005 и попадаем в админ-панель Nginx. Проект открытый, [почитать можно тут.](https://nginxproxymanager.com/guide/) Пользователь/пароль по умолчанию: `admin@example.com` и `changeme`.
