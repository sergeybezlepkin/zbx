...
**В каталоге проекта есть каталог logs/. Там лежит файл [compose.yml](logs/compose.yml) - отличного открытого ПО по сбору журналов с запущенных контейнеров, и все данные отображаются через браузер. Проект открытый с [ним можно ознакомится тут.](https://dozzle.dev/)**
